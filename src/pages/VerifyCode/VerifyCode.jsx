...
import {verifyOTPSignUp} from "../../hooks/useAuth";
...
const VerifyCode = () => {
    ...
    const verify = useMutation(verifyOTPSignUp);
    ...
    useEffect(() => {
        const error = verify?.data?.data?.error;

        if (error === false) {

  
            localStorage.setItem("userId", verify?.data?.data?.data?.id)
            localStorage.setItem("userName", verify?.data?.data?.data?.firstName + " ",
                verify?.data?.data?.data?.lastName)
            localStorage.setItem("userEmail", verify?.data?.data?.data?.email)
            localStorage.setItem("userPic", verify?.data?.data?.data?.picture)
            const data = verify?.data?.data?.data?.accessToken;
            const token = SignupToken(data);

            navigate("/conversation");
            isLoading(!loadingState)
        } else if (error === true) {
            isLoading(!loadingState)
            show(verify?.data?.data.message);
        }
    }, [verify.isSuccess]);
                    ...
                    <div
                        ...
                        >
                            <div>
                                ...
                                <div className="flex gap-5 pt-6">
                                    ...
                                    {otp.map((digit, index) => (
                                        <input
                                            key={index}
                                            className="border-[2px] inputItem0 border-solid w-[50px]  h-[50px] bg-[#E7EEF9]  text-[22px] xxl:text-[30px]  text-center text-[#AEBFF2] focus:outline-[#AEBFF2] rounded-full "
                                            type="text"
                                            maxLength="1"
                                            value={digit}
                            ...
                            <div className=" flex flex-col gap-3">
                                {/* <AuthButtonComp
                toSecond="/forgot-password"
                to="/create-new-password"
                firstButtonLabel="Verify"
